...
By the end of the day we had a clusters of similar ideas
...
2. Finalize some thing we could build together in the duration of the sprint.

To address (2) we had everyone vote on three clusters they'd like to work on that week. the Metadata and Taxonomy clusters emerged as victorious. This was most likely because those were the clusters we could make some progress on with the talent we had in that room and didn't require external help.
...
3. Looking through fact check stories about image manipulation. We also did a prototype claims review API client to see if there was a way to programatically search for stories about DMMs on fact check sites
